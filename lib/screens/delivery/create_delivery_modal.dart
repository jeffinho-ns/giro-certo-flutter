...
class _CreateDeliveryModalState extends State<CreateDeliveryModal> {
  ...
  Future<void> _fetchQuote() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Nao foi possivel calcular a taxa agora. Tente novamente em alguns segundos. ($e)',
          ),
  ...
  Future<void> _createOrder() async {
    ...
    if (_selectedPlaceId == null ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content:
              Text('Selecione um endereço válido da busca para continuar.'),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    try {
      ...
      if (user?.partnerId != null) {
        ...
        try {
          ...
          final is403 =
              errorMessage.contains('403') || errorMessage.contains('restrito');
          ...
          if (is403) {
            ...
            final tempPartner = Partner(
              id: user!.partnerId!,
              name: user
                  .name, // Usar nome do usuário como nome da loja temporariamente
            ...
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text(
                      'Atenção: Alguns dados da loja podem estar incompletos. Entre em contato com o suporte.'),
            ...
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content:
                      Text('Erro ao carregar dados da loja. Tente novamente.'),
                  backgroundColor: Colors.red,
  ...
  void _startAddressSearch(String value) {
    ...
    _addressDebounce = Timer(const Duration(milliseconds: 350), () async {
      ...
      try {
        ...
        setState(() {
          _addressPredictions.clear();
          _addressSearchError = message.isEmpty
              ? 'Falha ao buscar enderecos. Verifique sua conexao e tente novamente.'
              : message;
        });
      } finally {
  ...
  Widget _buildPriorityChip(
    ...
    return ChoiceChip(
      avatar: Icon(icon,
          size: 14,
          color: selected ? color : theme.iconTheme.color?.withOpacity(0.8)),
      label: Text(label),
      selected: selected,
      selectedColor: color.withOpacity(0.18),
      side: BorderSide(color: color.withOpacity(selected ? 0.55 : 0.25)),
      onSelected: (_) {
        setState(() {
                    ...
                    child: Column(
                      ...
                      children: [
                            ...
                            : _stores.isEmpty
                                ? Container(
                                    ...
                                    decoration: BoxDecoration(
                                      ...
                                      boxShadow:
                                          AppColors.insetPanelShadows(isDark),
                                    ),
                                    child: Text(
                                      'Carregando loja...',
                                ...
                                : _stores.length == 1
                                    ...
                                    DropdownButtonFormField<Partner>(
                                        value: _selectedStore != null &&
                                                _stores.contains(_selectedStore)
                                            ? _selectedStore
                                            : (_stores.isNotEmpty
                                                ? _stores.first
                                                : null),
                        ...
                        if (_addressSearchError != null &&
                            _addressSearchError!.isNotEmpty)
                          Padding(
                          ...
                          Container(
                            ...
                            child: ConstrainedBox(
                              ...
                              child: ListView.separated(
                                shrinkWrap: true,
                                itemCount: _addressPredictions.take(5).length,
                        ...
                        if (!_isSearchingAddress &&
                          ...
                          Padding(
                            padding: const EdgeInsets.only(top: 8),
                            child: Text(
                              'Nenhum endereco encontrado para esta busca.',
                              style: theme.textTheme.bodySmall?.copyWith(
                                color: theme.textTheme.bodyMedium?.color
                                    ?.withOpacity(0.75),
                              ),
                        ...
                        Wrap(
                          ...
                          runSpacing: 8,
                          children: [
                            _buildPriorityChip(theme, DeliveryPriority.low,
                                'Baixa', LucideIcons.circle),
                            _buildPriorityChip(theme, DeliveryPriority.normal,
                                'Normal', LucideIcons.circle),
                            _buildPriorityChip(theme, DeliveryPriority.high,
                                'Alta', LucideIcons.circle),
                            _buildPriorityChip(theme, DeliveryPriority.urgent,
                                'Urgente', LucideIcons.alertCircle),
                          ],
                        ),
