...
public class FlutterMapboxNavigationView : NavigationFactory, FlutterPlatformView
{
    ...
    init(messenger: FlutterBinaryMessenger, frame: CGRect, viewId: Int64, args: Any?)
    {
        ...
        self.channel.setMethodCallHandler { [weak self](call, result) in
            ...
            else if(call.method == "startNavigation")
            {
                ...
                strongSelf.recenterEmbeddedNavigationCamera(result: result)
            }
            else
            {
    ...
    private func setupMapView()
    {
        ...
        if(self.arguments != nil)
        {
            ...
            // Nunca usar CLLocation! sem atribuir: sem permissão rebentava ao ler currentLocation.
    ...
    func startEmbeddedNavigation(arguments: NSDictionary?, result: @escaping FlutterResult) {
        guard let response = self.routeResponse else {
            result(FlutterError(code: "NO_ROUTE", message: "Sem rota para iniciar navegacao.", details: nil))
            return
        }
        ...
        // O plugin original fazia `rootViewController as! FlutterViewController`, o que rebenta
        // com UINavigationController, TabBar, multi-window ou delegate.window nil (iOS 13+).
        guard let flutterViewController = Self.findFlutterViewController() else {
            result(FlutterError(
                code: "NO_FLUTTER_VC",
                message: "Nao foi possivel localizar FlutterViewController para anexar a navegacao.",
                details: nil
            ))
            return
        }
    ...
    /// Localiza o FlutterViewController na hierarquia (raiz da key window ou filhos).
    ...
    private static func findFlutterViewController(from vc: UIViewController) -> FlutterViewController? {
        if let flutter = vc as? FlutterViewController {
            return flutter
        }
    ...
    func recenterEmbeddedNavigationCamera(result: @escaping FlutterResult) {
