...
open class TurnByTurn(
    ...
    private fun recenterMapCamera() {
        activity.runOnUiThread {
            ...
            binding.root.findViewById<View>(R.id.recenter)?.performClick()
                ?: findRecenterButton(binding.navigationView)?.performClick()
        }
    }
    ...
    private fun findRecenterButton(root: View): View? {
        if (root.id == R.id.recenter) return root
        if (root is ViewGroup) {
            for (index in 0 until root.childCount) {
                findRecenterButton(root.getChildAt(index))?.let { return it }
            }
        }
        return null
    }
